...
func main() {
	...
	for {
		...
		mid1 := MiddleLayer1(in)
		mid2 := MiddleLayer2(in)
		mid3 := MiddleLayer3(in)
		mid := []float64{mid1,mid2,mid3}

		out1 := OutPutLayer1(mid)
		out2 := OutPutLayer2(mid)


		e21 := ErrorFunc(out1,b[int(rand)])
		e22 := ErrorFunc(out2,b[int(rand)])
		e2 := []float64{e21,e22}
		e11 := MiddleErrorFunc1(e2,mid1)
		e12 := MiddleErrorFunc2(e2,mid2)
		e13 := MiddleErrorFunc3(e2,mid3)

		h11 := function.InnerProduct(in,wm1)
		h12 := function.InnerProduct(in,wm2)
		h13 := function.InnerProduct(in,wm3)
...
func middleLayer(w, x []float64) (y float64) {
	h := innerProduct(w, x)
	y = sigmoid(h)
	return
}
